...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		// 初始化一个EncodedResource的方法
		// XMlBeanFactory方式的容器加载和ApplicationContext方式的容器加载在这里重合了， so 继续
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		// 使用apache的logging日志框架
		if (logger.isTraceEnabled()) {
			logger.trace("Loading XML bean definitions from " + encodedResource);
		}
		// 使用NamedThreadLocal 进行控制并发，最后清除数据
		Set<EncodedResource> currentResources = this.resourcesCurrentlyBeingLoaded.get();
		if (currentResources == null) {
			logger.info("第一次初始化的时候resourcesCurrentlyBeingLoaded中的Set<EncodedResource> 为null");
		...
		try {
			...
			try {
				// 根据原编码，初始化一个InputSource，用于正在的初始化加载Bean
				// 后续需要使用javax.xml.parsers.DocumentBuilder进行XML解析，需要org.xml.sax.InputSource对象
			...
			// 当线程退出的时候需要做的事， 先清除Set，在清除ConcurrentHashMap
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		return this.documentLoader.loadDocument(inputSource, getEntityResolver(), this.errorHandler,
				// 获取验证模式DTD（Document Type Definition）或者XSD（Xml Schema Definition）
	...
	protected int getValidationModeForResource(Resource resource) {
		...
		// 默认为VALIDATION_AUTO， 不等于，表示被设置过则直接返回
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
			// 使用XmlValidationModeDetector 进行自动检测
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 使用Spring的BeanUtils，调用ReflectionUtils.makeAccessible(DefaultBeanDefinitionDocumentReader.class)
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		// 记录加载前的BeanDefinition个数
		int countBefore = getRegistry().getBeanDefinitionCount();
		// 创建xml类型的读取上下文，会初始化类加载器，使用spi机制加载
		// SimpleConstructorNamespaceHandler，SimplePropertyNamespaceHandler，UtilNamespaceHandler
		// 注册beanDefinitions,也就是spring的Bean
		// createReaderContext, 使用组合模式，创建XmlReaderContext对象
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		// 返回注册的BeanDefinition个数
