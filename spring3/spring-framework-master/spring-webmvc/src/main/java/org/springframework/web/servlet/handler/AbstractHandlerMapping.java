...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	 ...
	 *
	 *  根据HttpServletRequest在当前的HandlerMapping中，获取对应的HandlerExecutionChain
	 *
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		...
		if (handler == null) {
			// 没有获取到再获取默认的，默认为空，若没有进行注册
			handler = getDefaultHandler();
		}
		// 前两步都没获取到，那就返回空（在下一个HandlerMapping中进行获取）
		if (handler == null) {
			return null;
		}
		// Bean name or resolved handler?
		// 如果为BeanName类型的，则直接在BeanFactory中获取该FactoryBean
		if (handler instanceof String) {
			String handlerName = (String) handler;
			handler = obtainApplicationContext().getBean(handlerName);
		}

		// 根据当前的handlerMapping和request，获取调用链（不仅根据request获取HandlerMapping，还需要获取所有匹配的Interceptor）
		...
		// 是否为Cors协议的请求（跨域的协议），则需要特殊处理
		if (CorsUtils.isCorsRequest(request)) {
			// 先获取全局的，全局不为null则合成当前的Cors配置，否则使用
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		// 转化为HandlerExecutionChain对象，HandlerMapping为HandlerExecutionChain的一个属性
		HandlerExecutionChain chain = (handler instanceof HandlerExecutionChain ?
				(HandlerExecutionChain) handler : new HandlerExecutionChain(handler));
		// 根据UrlPathHelper获取当前请求的请求路径， 如 http://www.jddj.com/user/userList 并且当前contextPath为空，则最后拿到 user/userList
		String lookupPath = this.urlPathHelper.getLookupPathForRequest(request);
		// 遍历所有的拦截器
		for (HandlerInterceptor interceptor : this.adaptedInterceptors) {
			// MappedInterceptor类型的拦截器则进行处理，否则直接添加到HandlerExecutionChain
			if (interceptor instanceof MappedInterceptor) {
	...
	protected CorsConfiguration getCorsConfiguration(Object handler, HttpServletRequest request) {
		...
		// 如果为HandlerExecutionChain类型，则获取合成HandlerExecutionChain前的HandlerMapping本身，否则可能本身就是一个HandlerMapping
		if (handler instanceof HandlerExecutionChain) {
			resolvedHandler = ((HandlerExecutionChain) handler).getHandler();
		}
		// 一切都在getCorsConfiguration中
