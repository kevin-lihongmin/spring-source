...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	 ...
	 *
	 *  1、查看当前的WebApplicationContext是否有被初始化过（初始化过就会保存到ServletContext容器中）
	 *  2、没有拿到，去ServletContext中获取一下
	 *  3、实在不行就自己创建一下，反正，会执行
	 *
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		// 再一次对容器进行充分的初始化，因为之前已经初始化的只是基本的ApplicationContext容器，现在需要将Servlet
		// 或者Spring的Servlet（DispatcherServlet）等信息，包括九大件的容器等进行充分的初始化
		...
		// 使用contextAttribute可再进行初始化容器，主要用于子实现
		if (wac == null) {
		...
		if (!this.refreshEventReceived) {
			...
			// SpringMVC的最后，初始化传说中的SpringMVC 9大件
			onRefresh(wac);
		}
	...
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		// 获取需要初始化的WebApplicationContext类型
		...
		// 老套路获取configLocation以初始化Resource
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		// 最后都会使用AbstractApplicationContext的refresh方法进行初始化，一个非常重要的类
		wac.refresh();
	}
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// LocaleContext 和 ServletRequestAttributes的前置处理
