...
public class UrlPathHelper {
	...
	public String getLookupPathForRequest(HttpServletRequest request) {
		...
		if (this.alwaysUseFullPath) {
			// http://www.jddj.com/user/userList?a=1    =>    (servlet context路径) + user/userList
	...
	public String getPathWithinApplication(HttpServletRequest request) {
		...
		if (path != null) {
			...
			// 正常合并完后，path不为空，则应该合成后的路径包含contextPath， 否则还有一种情况就是请求根路径
	...
	public String getRequestUri(HttpServletRequest request) {
		// 一样先从attribute中获取javax.servlet.include.request_uri
		String uri = (String) request.getAttribute(WebUtils.INCLUDE_REQUEST_URI_ATTRIBUTE);
		if (uri == null) {
			// 如浏览器中获取到： Request URL: https://www.baidu.com/ 去掉主机部分，获取第一个"/"开头的位置，玩字符串截取
	...
	public String getContextPath(HttpServletRequest request) {
		// 从attribute中获取javax.servlet.include.context_path
	...
	public String getServletPath(HttpServletRequest request) {
		// 先从attribute中获取javax.servlet.include.servlet_path
		String servletPath = (String) request.getAttribute(WebUtils.INCLUDE_SERVLET_PATH_ATTRIBUTE);
		if (servletPath == null) {
			// 获取servlet的路径
			servletPath = request.getServletPath();
		}
		// 以"/"结尾，并且应该去掉最后的不要的字符，则去掉最后的"/"
	...
	public String decodeRequestString(HttpServletRequest request, String source) {
		// 默认都需要解码
		if (this.urlDecode) {
			// 1、request携带的解密方式 2、ISO-8859-1 3、使用URLDecoder
	...
	private String decodeInternal(HttpServletRequest request, String source) {
		// 获取request中的加解密方式, 若request有带加解密方式，则使用该方式；否则使用默认的ISO-8859-1
	...
	public String removeSemicolonContent(String requestUri) {
		// 默认移除请求路径；后的字符串，否则只移除Jsessionid部分
		return (this.removeSemicolonContent ?
