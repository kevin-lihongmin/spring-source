...
public class ContextLoader {
	 ...
	 *  查询WebApplicationContext的实现类， 可以查看XmlWebApplicationContext的默认配置等
	...
	static {
		...
		try {
			// 加载WebApplicationContext策略， 默认会加载XMLWebApplicationContext， DEFAULT_STRATEGIES_PATH
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		...
		try {
			...
			if (this.context == null) {
				// 初始化容器，跟new ClassPathXmlApplicationContext("");一样
				this.context = createWebApplicationContext(servletContext);
			}
			// 一般情况下我们初始化XmlWebApplicationContext是ConfigurableWebApplicationContext的子类
			if (this.context instanceof ConfigurableWebApplicationContext) {
				ConfigurableWebApplicationContext cwac = (ConfigurableWebApplicationContext) this.context;
				// 在Ioc容器初始化的时候会有改标示的设置，现在肯定是false
				if (!cwac.isActive()) {
					...
					if (cwac.getParent() == null) {
						...
						// 老套路初始化父容器，里面会有很多的初始值，什么环境呀，，，，
						ApplicationContext parent = loadParentContext(servletContext);
						cwac.setParent(parent);
					}
					// 好吧，又回到ClassPathXmlApplicationText的线路
					configureAndRefreshWebApplicationContext(cwac, servletContext);
				}
			}
			// 将Spring容器放入全局的servletContext中，key为ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		// 先获取当前初始化的WebApplicationContext类型的class，默认会使用spi加载XMLWebApplicationContext
		Class<?> contextClass = determineContextClass(sc);
		if (!ConfigurableWebApplicationContext.class.isAssignableFrom(contextClass)) {
			throw new ApplicationContextException("Custom context class [" + contextClass.getName() +
					"] is not of type [" + ConfigurableWebApplicationContext.class.getName() + "]");
		}
		// 使用强大的spring的反射Util初始化一个容器
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		// 获取当前启动的WebApplicationContext的实现类
		...
		if (contextClassName != null) {
			...
			// 使用Hashtable作为策略,　没有加载到则使用默认策略，加载XmlWebApplicationContext容器
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		...
		// 老套路获取configLocation，再去初始化Resource什么的
	...
	protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
		// 拿到需要初始化的ConfigurableApplicationContext，不过他用的数组
		List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>> initializerClasses =
				determineContextInitializerClasses(sc);
		// 初始化不同类型的初始化ApplicationContextInitializer容器
